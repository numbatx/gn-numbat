...
// NewNumbatLogger will setup the defaults of the application logger.
...
func NewNumbatLogger(opts ...Option) *Logger {
	el := &Logger{
		logger:          log.New(),
...
func (el *Logger) ApplyOptions(opts ...Option) error {
	for _, opt := range opts {
		err := opt(el)
		if err != nil {
			fmt.Println("ssssssssssss", err)
			return errors.New("error applying option: " + err.Error())
		}
	}
	fmt.Println("ssssssssssss", el)
