...
func TestWithOptions(t *testing.T) {
	var str bytes.Buffer
	log := logger.NewNumbatLogger(logger.WithStackTraceDepth(1), logger.WithFile(&str))
...
func TestLazyFileWriter_WriteSomeLinesBeforeProvidingFile(t *testing.T) {
	var str bytes.Buffer
	expectedString := "this is a warning"
	log := logger.NewNumbatLogger(logger.WithStackTraceDepth(1))
	log.Warn(expectedString)
	log.ApplyOptions(logger.WithFile(&str))
	log.Warn("this is a warning")
	fmt.Println("11111111111111111111111111", str.String(), expectedString)
